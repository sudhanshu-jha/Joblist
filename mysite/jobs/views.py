...
def save_job_form(request, form, template_name):
    ...
    if request.method == 'POST':
        if form.is_valid():
            ...
            jobs = Job.objects.all()
            data['html_job_list'] = render_to_string('jobs/includes/partial_job_list.html', {
                'jobs': jobs
            })
        else:
...
def job_delete(request, pk):
    ...
    if request.method == 'POST':
        ...
        context = {'job': job}
        data['html_form'] = render_to_string('jobs/includes/partial_job_delete.html', context, request=request)
    return JsonResponse(data)
